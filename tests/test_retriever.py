...
def store_with_facts() -> FactStore:
    ...
    for f in facts:
        ...
        if f.app_name == "salesforce":
            now = time.time()
            for i in range(3):
                obs = FactObservation(fact_id=f.id, observed_at=now - i * 10, confirmed=True)
...
class TestFactRetriever:
    def test_query_filters_by_app_name(self, store_with_facts: FactStore) -> None:
    ...
    def test_query_filters_by_version(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        pairs = retriever.query(app_name="salesforce", app_version="2025.11", min_score=0.0)
        assert len(pairs) == 2

    def test_query_excludes_wrong_version(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        pairs = retriever.query(app_name="salesforce", app_version="2024.01", min_score=0.0)
        assert len(pairs) == 0

    def test_query_min_score_filters(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
    ...
    def test_query_sorted_by_score_descending(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        pairs = retriever.query(app_name="salesforce", min_score=0.0)
        scores = [s.score for _, s in pairs]
        assert scores == sorted(scores, reverse=True)

    def test_query_element_filter(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        pairs = retriever.query(app_name="salesforce", element="export", min_score=0.0)
        assert len(pairs) == 1
        assert pairs[0][0].element == "export-csv-button"

    def test_query_element_filter_case_insensitive(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        pairs = retriever.query(app_name="salesforce", element="EXPORT", min_score=0.0)
        assert len(pairs) == 1
    ...
    def test_bootstrap_context_contains_app(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        ctx = retriever.bootstrap_context("salesforce", "2025.11")
        assert "salesforce" in ctx

    def test_bootstrap_context_contains_facts(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        ctx = retriever.bootstrap_context("salesforce", "2025.11")
        assert "export-csv-button" in ctx

    def test_bootstrap_context_empty_store(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        ctx = retriever.bootstrap_context("unknown-app")
        assert "No known UI facts" in ctx or "unknown-app" in ctx

    def test_bootstrap_context_has_scores(self, store_with_facts: FactStore) -> None:
        retriever = FactRetriever(store_with_facts)
        ctx = retriever.bootstrap_context("salesforce", "2025.11")
    ...
    def test_retriever_uses_custom_scorer(self, store_with_facts: FactStore) -> None:
